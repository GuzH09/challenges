...
function calculatePrice(ornaments: string): number {
  ...
  for (let index=0; index<ornaments.length; index++) {
    const currentOrnamentValue = values[ornaments[index]];
    const nextOrnamentValue = values[ornaments[index + 1]];

    if ( currentOrnamentValue < nextOrnamentValue ) {
      sum -= currentOrnamentValue
    } else { // currentOrmentValue >= nextOrnamentValue || nextOrnamentValue == undefined
      sum += currentOrnamentValue
    }
  }
